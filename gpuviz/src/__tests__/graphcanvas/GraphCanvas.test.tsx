...
describe('GraphCanvas Component', () => {
  ...
  test('should show loading state initially', () => {
    act(() => {
      useGpuStore.setState({ loading: true, error: null, currentGraph: null });
    });
    render(<GraphCanvas />);
  ...
  test('should show error message when there is an error', () => {
    act(() => {
      useGpuStore.setState({ loading: false, error: 'Test Error', currentGraph: null });
    });
    render(<GraphCanvas />);
  ...
  test('should show empty message when there is no graph data', () => {
    act(() => {
      useGpuStore.setState({ loading: false, error: null, currentGraph: { nodes: [], edges: [] } });
    });
    render(<GraphCanvas />);
  ...
  describe('Lifecycle and Initialization', () => {
    ...
    beforeEach(() => {
      ...
      mockGraphCore.prototype.getCytoscapeInstance = jest.fn().mockReturnValue(mockCytoscapeInstance);
    ...
    test('should initialize GraphCore and GraphEvents on mount', () => {
      ...
      expect(mockGraphCore.prototype.getCytoscapeInstance).toHaveBeenCalledTimes(1);
      expect(mockGraphEvents.prototype.setCytoscapeInstance).toHaveBeenCalledWith(mockCytoscapeInstance);
      expect(mockGraphEvents.prototype.on).toHaveBeenCalledWith('nodeClick', expect.any(Function));
      expect(mockGraphEvents.prototype.on).toHaveBeenCalledWith('canvasClick', expect.any(Function));
    });
    ...
    test('should update graph when currentGraph data changes in the store', () => {
        ...
        expect(mockGraphCore.prototype.updateGraph).toHaveBeenCalledWith(initialState.currentGraph);
        ...
        expect(mockGraphCore.prototype.updateGraph).toHaveBeenCalledWith(newGraph);
    });
  });
