...
describe('GraphConfig', () => {
  ...
  describe('GRAPH_CONFIG', () => {
    ...
    it('should contain valid color constants', () => {
      ...
      expect(GRAPH_CONFIG.DEFAULT_NODE_BORDER_COLOR).toMatch(/^#([0-9a-f]{3}){1,2}$/i);
      expect(GRAPH_CONFIG.SELECTED_NODE_COLOR).toMatch(/^#([0-9a-f]{3}){1,2}$/i);
      expect(GRAPH_CONFIG.SELECTED_NODE_BORDER_COLOR).toMatch(/^#([0-9a-f]{3}){1,2}$/i);
    });
  });
  ...
  describe('DEFAULT_GRAPH_OPTIONS', () => {
    ...
    it('should reference the default styles and a valid layout', () => {
      ...
      expect(Object.values(LAYOUT_PRESETS)).toContain(DEFAULT_GRAPH_OPTIONS.layout);
    });
