...
export class ComponentNodeBuilder<T extends NodeInfo> {
   ...
   private ensureParentExists(componentName: string, tempComponentList: ComponentNode<T>[], tempComponentNames: Set<string>): void {
        ...
        if (this.componentMap.has(parentName) || tempComponentNames.has(parentName)) {
            return; 
        }
